...
def evaluate_output(scorer_path, gold_filepath, out_filepath):
    print(scorer_path)
    eval_cmd = ['java','-cp', scorer_path, scorer_path+'/Scorer.java', gold_filepath, out_filepath]
...
def normalize_length_for_context(ids, attn_mask, o_mask, m_o_mask, max_len, pad_id):
    if max_len == -1:
        return ids, attn_mask, o_mask, m_o_mask
    else:
        if len(ids) < max_len:
