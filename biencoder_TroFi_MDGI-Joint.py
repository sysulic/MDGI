...
def load_VUA_data(trofi_file):
    loaded_data = []

    with open(trofi_file, 'r') as f:
        lines = csv.reader(f)
        next(lines)
        count = 0
        for line in lines:
            ...
            for w_idx, w in enumerate(sent):
                if w_idx == int(verb_idx):
                    sentence_detail.append((w, label, m_label))
                else:
                    sentence_detail.append((w, -1, 0))
...
def train_model(args):
    ...
    train_path = os.path.join(args.data_path, 'trofi_train.csv')
    train_data = load_VUA_data(train_path)
    gloss_file = os.path.join(args.data_path, 'gloss/all_meaning.csv')
    ...
    if not args.train_w:
        model_path = os.path.join(args.ckpt, 'trofi_na_best_model_context.ckpt')
    ...
    #loading WSD (semcor) data
    dev_path = os.path.join(args.data_path, 'trofi_val.csv')
    ...
    for epoch in range(1, epochs+1):
        ...
        pred_filepath = os.path.join(args.ckpt, 'trofi_tmp_predictions.txt')
        ...
        if dev_f1 >= best_dev_f1:
            ...
            model_fname = os.path.join(args.ckpt, 'trofi_na_best_model_context.ckpt')
...
def evaluate_model(args):
    ...
    #loading WSD (semcor) data
    eval_path = os.path.join(args.data_path, 'trofi_test.csv')
    ...
    f_m_label, f_m_pred = [], []
    for m_label, m_mark, m_pred in zip(eval_metaphor_labels, eval_marks, eval_m_preds):
        if m_mark == 1:
            f_m_label.append(m_label)
            f_m_pred.append(m_pred)
    ...
    #generate predictions file
    pred_filepath = os.path.join(args.ckpt, 'trofi_test_predictions.txt')
    with open(pred_filepath, 'w') as f:
        for m_l, prediction, mark in zip(eval_metaphor_labels, eval_m_preds, eval_marks):
            f.write('{} {} {}\n'.format(m_l, prediction, mark))

    pred_filepath = os.path.join(args.ckpt, 'trofi_test_predictions_para.txt')
    with open(pred_filepath, 'w') as f:
        for label, prediction in zip(eval_label_glosses, eval_preds):
            f.write('{} {}\n'.format(m_l, prediction))
