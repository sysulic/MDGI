...
def read_glosses_from_file(file):
    gloss_dict = {}
    ...
    for row in range(1, rows):
        word = data.cell_value(row, 0)
        defs = data.cell_value(row, 1)
        if word in gloss_dict:
            gloss_dict[word].append(defs)
        else:
            gloss_dict[word] = ['NULL', defs]
    return gloss_dict

def get_label_glosses(label_file):
    label_glosses = []
    with open(label_file, 'r', encoding='GB18030') as f:
        lines = csv.reader(f)
        next(lines)
        for line in lines:
            gloss_labels = ast.literal_eval(line[6])
            for g in gloss_labels:
                if g != 'NULL':
                    label_glosses.append(g)
...
def get_metaphor_label(label_file):
    ...
    with open(label_file, 'r', encoding='GB18030') as f:
        ...
        for line in lines:
            ...
            label_seq = ast.literal_eval(line[5])
            pos_seq = ast.literal_eval(line[4])
            assert (len(pos_seq) == len(label_seq))
            assert (len(line[3].split()) == len(pos_seq))
            sent = line[3].split(' ')
...
def load_VUA_data(VUA_file):
    ...
    with open(VUA_file, 'r', encoding='GB18030') as f:
        lines = csv.reader(f)
        next(lines)
        count = 1
        for line in lines:
            ...
            assert (len(line[3].split()) == len(pos_seq))
            assert (len(line[3].split()) == len(label_seq))
            
            for w_idx, w in enumerate(sent):
                if gloss_labels[w_idx] != 'NULL':
                    sentence_detail.append((w, gloss_labels[w_idx], int(label_seq[w_idx])))
                else:
...
def train_model(args):
    ...
    train_path = os.path.join(args.data_path, 'PSUCMC_sen_train.csv')
    train_data = load_VUA_data(train_path)
    gloss_file = os.path.join(args.data_path, 'gloss/PSUCMC_all_meaning.xlsx')
    ...
    if not args.train_w:
        model_path = os.path.join(args.ckpt, 'psucmc_na_best_model_context.ckpt')
    ...
    #loading WSD (semcor) data
    dev_path = os.path.join(args.data_path, 'PSUCMC_sen_val.csv')
    ...
    dev_metaphor_labels, dev_marks = get_metaphor_label(dev_path)
    #load gloss dictionary (all senses from webster dictionary)
    ...
    for epoch in range(1, epochs+1):
        ...
        pred_filepath = os.path.join(args.ckpt, 'psucmc_tmp_predictions.txt')
        ...
        if dev_f1 >= best_dev_f1:
            ...
            model_fname = os.path.join(args.ckpt, 'psucmc_na_best_model_context.ckpt')
...
def evaluate_model(args):
    ...
    #loading WSD (semcor) data
    eval_path = os.path.join(args.data_path, 'PSUCMC_sen_test.csv')
    ...
    eval_metaphor_labels, eval_marks = get_metaphor_label(eval_path)
    #load gloss dictionary (all senses from webster dictionary)
    ...
    #generate predictions file
    pred_filepath = os.path.join(args.ckpt, 'psucmc_test_predictions.txt')
    with open(pred_filepath, 'w') as f:
        for m_l, prediction, mark in zip(eval_metaphor_labels, eval_m_preds, eval_marks):
            f.write('{} {} {}\n'.format(m_l, prediction, mark))
